...
// Se 'html' é usado em outros arquivos, 'extern' é correto.
// Se não, pode remover a linha abaixo.
...
void preencher_html() {
    snprintf(html, sizeof(html),
        ...
        "        .rack-cell {background-color: #e9ecef;border: 1px solid #ccc;border-radius: 4px;aspect-ratio: 1 / 1; display: flex;align-items: center;justify-content: center;font-weight: bold;font-size: 0.9rem;transition: background-color 0.3s ease, transform 0.2s ease;cursor: pointer;}\n"
        ...
        "                if (currentSlot) {\n"
        "                    // 1. Guarda o valor atual para usar na requisição e no log\n"
        "                    const slotToRemove = currentSlot;\n"
        "\n"
        "                    // 2. Encontra a célula na página e remove a classe 'occupied' IMEDIATAMENTE\n"
        "                    const cellElement = document.querySelector(`[data-position=\"${slotToRemove}\"]`);\n"
        "                    if (cellElement) {\n"
        "                        cellElement.classList.remove('occupied');\n"
        "                    }\n"
        "\n"
        "                    // 3. Envia a requisição para o servidor em segundo plano (assíncrono)\n"
        "                    fetch(`/retrieve?slot=${slotToRemove}`, { method: 'POST' })\n"
        "                        .then(response => {\n"
        "                            if (response.ok) {\n"
        "                                // Usa a variável local 'slotToRemove' que não foi alterada\n"
        "                                addToHistory(`Retirada da posição ${slotToRemove} enviada com sucesso.`);\n"
        "                            } else {\n"
        "                                addToHistory(`ERRO ao retirar da posição ${slotToRemove}.`);\n"
        "                                // Opcional: Se falhar, adiciona a classe de volta\n"
        "                                if(cellElement) cellElement.classList.add('occupied');\n"
        "                            }\n"
        "                        })\n"
        "                        .catch(error => {\n"
        "                            console.error('Erro de conexão:', error);\n"
        "                            addToHistory(`ERRO de conexão ao retirar ${slotToRemove}.`);\n"
        "                            if(cellElement) cellElement.classList.add('occupied'); // Reverte em caso de erro\n"
        "                        });\n"
        "                }\n"
        "                popupOverlay.style.display = 'none';\n"
        "                currentSlot = null; // Limpa a variável global depois de tudo\n"
        "            });\n"
        "\n"
        ...
        "                        addToHistory(`Eletroímã ${newStatus.toLowerCase()}.`);\n"
        "                    } else {\n"
        ...
        "            retrieveForm.addEventListener('submit', function(e) { e.preventDefault(); const pos = document.getElementById('pallet-position-input').value.trim().toUpperCase(); if (pos) { addToHistory(`Pallet retirado da posição ${pos}.`); document.getElementById('pallet-position-input').value = ''; } });\n"
